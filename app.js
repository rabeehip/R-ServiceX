...
if (UI.dark_mode) {
  document.write(
    '<link rel="stylesheet" href="//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/css/mdui-dark.min.css">',
  );
} else {
  document.write(
    '<link rel="stylesheet" href="//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/css/mdui.min.css">',
  );
}
if (UI.dark_mode) {
  document.write(
    '<link rel="stylesheet" href="//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/css/nexmoe-dark.min.css">',
  );
} else {
  document.write(
    '<link rel="stylesheet" href="//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/css/nexmoe.min.css">',
  );
}
document.write(
  '<script src="//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/js/mdui.min.js"></script>',
);
document.write(
  '<link rel="stylesheet" href="//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/css/video-js.min.css">',
);
document.write(
  '<script src="//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/js/flv.min.js"></script>',
);
document.write(
  '<script src="//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/js/video.min.js"></script>',
);
document.write(
  '<script src="//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/js/marked.min.js"></script>',
);
document.write(
...
function title(path) {
  if (path.slice(-1) !== "/") {
    path = decodeURIComponent(
      path.trim("/").split("/").slice(-1)[0].replaceAll("%5C%5C", "%5C"),
    );
  } else {
  ...
  var search_bar = `<div class="mdui-toolbar-spacer"></div>
        <div id="search_bar" class="mdui-textfield mdui-textfield-expandable mdui-float-right ${
          model.is_search_page ? "mdui-textfield-expanded" : ""
            ...
            <input class="mdui-textfield-input" type="text" name="q" placeholder="Search item(s)..." value="${search_text}"/>
            </form>
...
function list(path) {
  ...
  <i class="mdui-icon material-icons icon-sort" data-sort="size" data-order="downward">expand_more</i>
	 ...
	 <div class="mdui-row"> 
	  ...
	  <div id="count" class="mdui-hidden mdui-center mdui-text-center mdui-m-b-3 mdui-typo-subheading mdui-text-color-blue-grey-500">Total found(<span class="number"></span>)</div>
	 </div>
	`;
  ...
  for (i in files) {
    ...
    if (item.mimeType == "application/vnd.google-apps.folder") {
      ...
      var p =
        ...
        encodeURIComponent(item.name)
          .replaceAll("%5C", "%5C%5C")
          .replace(/[!'()*]/g, escape); // Adding file name to url
      var ddl_link = p;
      ...
      if (
        "|html|php|css|go|java|js|json|txt|sh|md|mp4|webm|avi|bmp|jpg|jpeg|png|gif|m4a|mp3|opus|flac|wav|ogg|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|pdf|".indexOf(
          `|${ext}|`,
        ) >= 0
      ) {
...
function append_search_result_to_list(files) {
  ...
  for (i in files) {
    ...
    if (item.mimeType == "application/vnd.google-apps.folder") {
      ...
      if (
        "|html|php|css|go|java|js|json|txt|sh|md|mp4|webm|avi|bmp|jpg|jpeg|png|gif|m4a|mp3|opus|flac|wav|ogg|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(
          `|${ext}|`,
        ) >= 0
      ) {
        c += " view";
...
function onSearchResultItemClick(a_ele) {
  ...
  if (!fileId) {
    mdui.dialog({
      title: 'Error',
      content: 'No file ID found. The clicked element might not be valid.',
      buttons: [{ text: 'Close' }],
      history: false,
      modal: true,
      closeOnEsc: true,
  ...
  $.post(`/${cur}:id2path`, { id: fileId }, function (data) {
    ...
    if (data) {
      ...
      mdui.dialog({
        title: `
           <div class="mdui-text-center mdui-typo-title"><i class="mdui-icon material-icons mdui-m-r-0">check_circle_outline</i> Choose your opted-for option</div>
        `,
        content: linkHTML,
        history: false,
        modal: true,
      ...
      mdui.dialog({
        ...
        content: `
          <p>Unable to retrieve the target path. Possible reasons include:</p>
          <ul>
            <li>The file no longer exists in your drive.</li>
            <li>Decryption of the file ID failed.</li>
            <li>The backend service could not resolve the file ID.</li>
            <li>The drive index may be outdated or out of sync.</li>
            <li>The item might not exist in the specified folder.</li>
            <li>The file resides in <strong>Shared with me</strong> and has not been added to your personal drive.</li>
          </ul>
        `,
        buttons: [{ text: "Close" }],
        history: false,
    ...
    mdui.dialog({
      title: 'Server Error',
      content: `Failed to contact the server. Status: ${textStatus}`,
      buttons: [{ text: "Close" }],
      history: false,
      modal: true,
      closeOnEsc: true,
...
function file(path) {
  ...
  if ("|mp3|flac|opus|wav|ogg|m4a|".indexOf(`|${ext}|`) >= 0) {
...
function file_code(path) {
  ...
  var content = `
    ...
    <script src="//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/js/ace.js"></script>
    <script src="//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/js/ext-language_tools.js"></script>
	`;
  $("#content").html(content);
...
<div class="mdui-container-fluid">
    ...
    <div class="mdui-typo-body-1 mdui-m-t-2">
       <span class="mdui-text-color-theme"><i class="mdui-icon material-icons">info_outline</i> File information</span>
    </div>
    <div class="mdui-textfield">
...
<button onclick="location.href='${url}'" title="Download Content" class="mdui-fab mdui-fab-fixed mdui-ripple mdui-color-theme-accent" aria-label="Download">
  ...
  $("#copy-link").on("click", () => {
    copyToClipboard(url);
    mdui.snackbar("Copied successfully.");
  });
  const player = videojs("video-id", {
    fluid: true, // Maintains responsive aspect ratio
    autoplay: false, // Prevents auto-play
    controls: true, // Displays playback controls
    playsinline: true, // iOS inline playback
    enableSmoothSeeking: true, // Optional plugin-based feature (custom or third-party)
    enableDocumentPictureInPicture: true, // Enables PiP if supported
    playbackRates: [0.25, 0.5, 1, 1.25, 1.5, 2], // Expanded rate options
    controlBar: {
      autoHide: true,
    },
    poster: "//cdn.jsdelivr.net/gh/rabeehip/R-ServiceX/images/poster.png", // Shown before playback starts
    errorDisplay: true, // Show errors in UI
    techOrder: ["html5", "youtube", "flash"], // Fallback options if needed
...
<div class="mdui-container-fluid">
	<br>
	<div class="mdui-video-fluid mdui-center">
	  <audio id="audio-id" class="video-js vjs-default-skin" controls preload="auto" width="100%" height="auto">
        <source src=${url} type="audio/mp3">
        Your browser does not support the audio tag or the file format of this audio.
      </audio>
    </div>
	<br>${playBtn}
	<!-- ???? -->
...
function file_pdf(path) {
  ...
  var display_name = decodeURIComponent(
    path.trim("/").split("/").slice(-1)[0].replaceAll("%5C%5C", "%5C"),
  );
  var content = `
